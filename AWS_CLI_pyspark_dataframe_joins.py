...
## Save it as a table and use SQL to show we get the same results.
md_prop_county.write.mode("overwrite").saveAsTable("mytable")
spark.sql('select County, sum(count_property) from mytable group by County order by County').show()
