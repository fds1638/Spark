...
# This uses multiple linear regression, which is not the best
# algorithm. But the purpose is to show the use of pyspark.
...
test_rdd = sc.textFile("kaggle/test.csv")
header = test_rdd.first()
test_rdd = test_rdd.filter(lambda r: r!=header)
test_rdd = test_rdd.map(lambda s: s.replace('""',''))
test_rdd = test_rdd.map(lambda s: s.replace('female','1'))
test_rdd = test_rdd.map(lambda s: s.replace('male','0'))
test_rdd = test_rdd.map(lambda s: s.replace(',,',',0,'))
test_rdd = test_rdd.map(lambda x: x.split("\""))
test_rdd = test_rdd.map(lambda x: [x[i] for i in [2]])
test_rdd = test_rdd.map(lambda x: x[0])
test_rdd = test_rdd.map(lambda x: x.split(","))
test_rdd = test_rdd.map(lambda x: [float(x[i])*1.0 for i in [1,2,6]])
